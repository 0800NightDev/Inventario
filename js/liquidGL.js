...
(() => {
  ...
  class liquidGLRenderer {
    ...
    async captureSnapshot() {
      ...
      if (typeof html2canvas === "undefined") {
        console.warn("liquidGL: html2canvas is not defined. Revealing pending lenses immediately.");
    ...
    _uploadTexture(srcCanvas) {
      ...
      if (srcCanvas) {
        ...
        } else if (srcCanvas.width > 0 && srcCanvas.height > 0) {
          success = true;
        }
      }
    ...
    addLens(element, options) {
      if (element.hasAttribute('data-liquid-initialized')) {
        const existing = this.lenses.find(l => l.el === element);
        if (existing) return existing;
      }
      element.setAttribute('data-liquid-initialized', 'true');
    ...
    render() {
      ...
      this.lenses = this.lenses.filter((lens) => {
        if (!lens.el.isConnected) {
          if (typeof lens.destroy === "function") {
            lens.destroy();
          }
          return false;
        }
  ...
  class liquidGLLens {
    ...
    destroy() {
      ...
      if (this.el && this.el.removeAttribute) {
        this.el.removeAttribute('data-liquid-initialized');
      }
    }
  }
   ...
   *  Public API
   * ------------------------------------------------*/
  window.liquidGL = function (selectorOrOptions = {}, userOptions = {}) {
    let options = {};
    if (typeof selectorOrOptions === "string") {
      options = { target: selectorOrOptions, ...userOptions };
    } else {
